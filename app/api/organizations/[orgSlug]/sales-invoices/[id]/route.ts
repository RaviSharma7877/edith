...
export async function PATCH(
  ...
  const { invoiceDate, dueDate, notes, terms, placeOfSupply, lines } = body

  const data: Record<string, unknown> = {}
  if (invoiceDate)    data.invoiceDate    = new Date(invoiceDate)
  if (dueDate)        data.dueDate        = new Date(dueDate)
  if ("notes" in body)  data.notes        = notes   ?? null
  if ("terms" in body)  data.terms        = terms   ?? null
  if (placeOfSupply)  data.placeOfSupply  = placeOfSupply

  if (lines && Array.isArray(lines)) {
    let subtotal = 0, taxAmount = 0
    type LineInput = { quantity?: string | number; unitPrice?: string | number; discountPct?: string | number; taxRate?: string | number; description?: string; hsnCode?: string | null; unit?: string | null; accountId?: string | null }
    const lineData = (lines as LineInput[]).map((l, idx: number) => {
      ...
      const disc = parseFloat(String(l.discountPct ?? "0")) || 0
      const taxR = parseFloat(String(l.taxRate ?? "0")) || 0
      const base = qty * rate * (1 - disc / 100)
      const tax  = base * (taxR / 100)
      subtotal  += base
      taxAmount += tax
      return { lineNumber: idx + 1, description: l.description ?? "", hsnCode: l.hsnCode ?? null,
        quantity: qty, unit: l.unit ?? null, unitPrice: rate, discountPct: disc || null,
    ...
    await prisma.$transaction([
      prisma.salesInvoiceLine.deleteMany({ where: { invoiceId: id } }),
  ...
  const updated = await prisma.salesInvoice.update({ where: { id }, data })
