...
interface VendorRow {
  vendorId: string; vendorName: string; vendorCode: string | null
  current: number; "1_30": number; "31_60": number; "61_90": number; over_90: number
  total: number; bills: BillRow[]
}
interface APData {
  asOf: string
  rows: VendorRow[]
...
export default function APAgingPage() {
  ...
  const load = useCallback(async (d: string) => {
    ...
    try {
      const res = await fetch(`/api/organizations/${orgSlug}/reports/ap-aging?asOf=${d}`)
    ...
    <div className="flex flex-col h-full">
      <header className="flex items-center gap-3 border-b border-[rgba(55,50,47,0.12)] bg-white px-6 py-4">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-semibold text-[#37322F]">AP Aging</h1>
          <a href={`/${orgSlug}/reports`} className="text-xs text-[#8B8580] hover:text-[#37322F]">← Reports</a>
        </div>
        <div className="flex items-center gap-2">
          <Label htmlFor="apAsOf" className="text-xs">As of</Label>
          <Input id="apAsOf" type="date" value={asOf} onChange={(e) => setAsOf(e.target.value)} className="h-8 w-36 text-sm" />
          ...
          <div className="w-full min-w-0">
            ...
            <div className="grid grid-cols-6 gap-3 mb-5">
              ...
              <div className="rounded-lg border border-[rgba(55,50,47,0.20)] bg-[#37322F] px-3 py-3">
                <p className="text-[10px] text-white/60 mb-1">Total payable</p>
                  ...
                  <tbody>
                    {data.rows.map((row) => (
                      <>
                        ...
                        >
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="font-medium text-[#37322F]">{row.vendorName}</span>
                              {row.vendorCode && (
                                <span className="text-[10px] text-[#8B8580] font-mono">{row.vendorCode}</span>
                              )}
                            </div>
                          </td>
                          {BUCKET_KEYS.map((k, i) => (
                        ...
                        {expanded.has(row.vendorId) && row.bills.map((b) => (
                          <tr key={b.id} className="bg-[#FAFAF9] border-t border-[rgba(55,50,47,0.04)]">
                            <td className="pl-10 pr-4 py-2">
                              <a href={`/${orgSlug}/purchase-bills/${b.id}`}
                            ...
                            {BUCKET_KEYS.map((k) => (
                              <td key={k} className={`px-3 py-2 text-right font-mono text-xs ${b.bucket === k ? BUCKET_COLORS[BUCKET_KEYS.indexOf(k)] : "text-[#C5BFB8]"}`}>
                                {b.bucket === k ? fmt(b.amountDue) : "—"}
                              </td>
                            ))}
