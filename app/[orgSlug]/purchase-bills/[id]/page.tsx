    ...
    <div className="flex flex-col h-full">
      <header className="flex items-center justify-between border-b border-[rgba(55,50,47,0.12)] bg-white px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-sm font-medium text-[#37322F]">{bill.billNumber}</span>
          {bill.isDebitNote && (
            <span className="rounded bg-blue-100 px-1.5 py-0.5 text-[10px] font-semibold text-blue-700">DEBIT NOTE</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <span className={`rounded px-2 py-1 text-xs font-medium ${STATUS_COLORS[bill.status] ?? "bg-gray-100 text-gray-700"}`}>
            {bill.status.replace(/_/g, " ")}
          </span>
          <BillActions orgSlug={orgSlug} billId={id} status={bill.status} isDebitNote={bill.isDebitNote} />
        </div>
      </header>
        ...
        <div className="w-full min-w-0 space-y-6">
          ...
          {bill.debitNoteOf && (
            <div className="rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
              Debit note for{" "}
              <Link href={`/${orgSlug}/purchase-bills/${bill.debitNoteOf.id}`} className="font-medium underline">
                {bill.debitNoteOf.billNumber}
              </Link>
            </div>
          )}
          ...
          {bill.debitNotes.length > 0 && (
            <div className="rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
              Debit notes:{" "}
              {bill.debitNotes.map((dn, i) => (
                <span key={dn.id}>
                  {i > 0 && ", "}
                  <Link href={`/${orgSlug}/purchase-bills/${dn.id}`} className="font-medium underline">
                    {dn.billNumber}
                  </Link>
                  {" "}
                  <span className={`inline-block rounded px-1 py-0.5 text-[9px] font-medium ${STATUS_COLORS[dn.status] ?? "bg-gray-100 text-gray-700"}`}>
                    {dn.status}
                  </span>
                </span>
              ))}
          ...
          <div className="rounded-lg border border-[rgba(55,50,47,0.10)] bg-white p-5">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div>
                <p className="text-xs text-[#605A57]">Vendor</p>
                <Link href={`/${orgSlug}/vendors/${bill.vendor.id}`} className="mt-0.5 text-sm font-medium text-[#37322F] hover:underline block">
                  {bill.vendor.name}
                </Link>
                {bill.vendor.gstin && (
                  <p className="text-[10px] font-mono text-[#605A57]">{bill.vendor.gstin}</p>
                )}
              </div>
              <div>
                <p className="text-xs text-[#605A57]">Bill date</p>
                <p className="mt-0.5 text-sm font-medium text-[#37322F]">{fmtDate(bill.billDate)}</p>
              </div>
              <div>
                <p className="text-xs text-[#605A57]">Due date</p>
                <p className="mt-0.5 text-sm font-medium text-[#37322F]">{fmtDate(bill.dueDate)}</p>
              </div>
              <div>
                <p className="text-xs text-[#605A57]">Vendor ref</p>
                <p className="mt-0.5 text-sm font-medium text-[#37322F]">{bill.vendorBillRef ?? "—"}</p>
              </div>
              {bill.placeOfSupply && (
                <div>
                  <p className="text-xs text-[#605A57]">Place of supply</p>
                  <p className="mt-0.5 text-sm text-[#37322F]">{bill.placeOfSupply}</p>
                </div>
              )}
              {bill.notes && (
                <div className="col-span-2 sm:col-span-4">
                  <p className="text-xs text-[#605A57]">Notes</p>
                  <p className="mt-0.5 text-sm text-[#37322F] whitespace-pre-wrap">{bill.notes}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-lg border border-[rgba(55,50,47,0.10)] bg-white overflow-hidden">
            <div className="grid grid-cols-[2.5fr_0.7fr_0.7fr_0.8fr_0.7fr_1fr] gap-3 border-b border-[rgba(55,50,47,0.10)] bg-[#F7F5F3] px-4 py-2 text-xs font-medium text-[#605A57]">
            ...
            {bill.lines.map((line) => (
              <div
                key={line.id}
                className="grid grid-cols-[2.5fr_0.7fr_0.7fr_0.8fr_0.7fr_1fr] items-start gap-3 border-b border-[rgba(55,50,47,0.06)] px-4 py-2.5 text-sm"
              >
                <div>
                ...
                <span className="text-right font-mono text-sm font-medium text-[#37322F]">₹{fmt(line.lineTotal)}</span>
              </div>
            ))}
