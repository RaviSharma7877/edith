...
export default async function ESIRegisterPage({ params }: { params: Promise<{ orgSlug: string }> }) {
        ...
        <div className="overflow-hidden rounded-lg border border-[rgba(55,50,47,0.10)] bg-white">
          ...
          {typedRuns.map((run) => (
            <div key={run.id} className="grid grid-cols-[1.2fr_1fr_1fr_1fr] items-center gap-3 border-t border-[rgba(55,50,47,0.06)] px-4 py-3 text-sm">
              ...
              <Link href={`/${orgSlug}/payroll/statutory/esi/${run.id}`} className="text-xs font-medium text-primary hover:underline">
                View →
              </Link>
            </div>
          ))}
