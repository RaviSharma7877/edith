...
import { deleteFixedAsset, updateFixedAsset } from "../../payroll/actions"
import { FixedAssetForm, Phase5Shell } from "../../payroll/_components/phase5-ui"

export default async function EditFixedAssetPage({ params }: { params: Promise<{ orgSlug: string; id: string }> }) {
  const { orgSlug, id } = await params
  const session = await getServerSession(authOptions)
  if (!session?.user?.email) redirect("/login")
  const ctx = await resolveCompany(orgSlug, session.user.email)
  if (!ctx) redirect("/workspace")
  const asset = await prisma.fixedAsset.findFirst({ where: { id, companyId: ctx.company.id, deletedAt: null } })
  if (!asset) notFound()
