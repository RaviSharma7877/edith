...
interface TDSEntry {
  ...
  vendor:        { id: string; name: string; pan: string | null }
}
...
export default function TDSDashboardPage() {
  ...
  const load = useCallback(async (q: string, s: string) => {
    ...
    setError(null)
    try {
      const params = new URLSearchParams({ quarter: q })
      if (s !== "all") params.set("status", s)
      const res = await fetch(`/api/organizations/${orgSlug}/tds/entries?${params}`)
  ...
  async function handleExport() {
    setExporting(true)
    try {
      const res = await fetch(`/api/organizations/${orgSlug}/tds/entries/export?quarter=${quarter}`)
      if (!res.ok) throw new Error("Export failed")
      const blob = await res.blob()
      const url  = URL.createObjectURL(blob)
      const a    = document.createElement("a")
      a.href     = url
      a.download = `Form26Q-${quarter}.json`
      a.click()
  ...
  const totalTDS  = entries.reduce((s, e) => s + Number(e.tdsAmount), 0)
  const deposited = entries.filter((e) => e.status === "deposited" || e.status === "filed").reduce((s, e) => s + Number(e.tdsAmount), 0)
  const pending   = entries.filter((e) => e.status === "deducted").reduce((s, e) => s + Number(e.tdsAmount), 0)
    ...
    <div className="flex flex-col h-full">
      <header className="flex items-center gap-3 border-b border-[rgba(55,50,47,0.12)] bg-white px-6 py-4">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-semibold text-[#37322F]">TDS — Tax Deducted at Source</h1>
          <p className="text-xs text-[#605A57]">Form 26Q quarterly filing</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" asChild>
            <a href={`/${orgSlug}/tds/entries/new`}>
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { label: "TDS deducted",    value: `₹${fmt(totalTDS)}`  },
            { label: "Deposited / filed", value: `₹${fmt(deposited)}` },
        ...
        {loading && !entries.length ? (
          ...
          <div className="rounded-lg border border-[rgba(55,50,47,0.12)] bg-white overflow-hidden">
            <table className="w-full text-xs">
              <thead>
                <tr>
                  {["Date", "Section", "Vendor", "PAN", "Base amount", "TDS amount", "Challan", "Status"].map((h) => (
                    <TH key={h}>{h}</TH>
                  ))}
                </tr>
              </thead>
              <tbody>
                {entries.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-3 py-8 text-center text-[#8B8580]">
                      No TDS entries for {quarter}
                    </td>
                  </tr>
                ) : entries.map((e) => (
                  <tr key={e.id} className="border-t border-[rgba(55,50,47,0.06)] hover:bg-[#FAFAF9]">
                    ...
                    <TD>{e.vendor.name}</TD>
                    <TD><span className="font-mono">{e.vendor.pan ?? "—"}</span></TD>
                    <TD right>₹{fmt(Number(e.baseAmount))}</TD>
                    <TD right>₹{fmt(Number(e.tdsAmount))}</TD>
